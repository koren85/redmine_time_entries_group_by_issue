  ...
  module TimeEntryQueryPatch
    def self.included(base)
      base.class_eval do
        ...
        alias_method :initialize_available_filters_without_parent, :initialize_available_filters if method_defined?(:initialize_available_filters)
        ...
        # Добавляем фильтр по родительским задачам
        def initialize_available_filters
          # Сначала вызываем оригинальный метод для инициализации стандартных фильтров
        ...
        def sql_for_parent_issue_id_field(field, operator, value)
          ...
          when "*"
            ...
            sql = "#{time_entry_table}.issue_id IN (SELECT id FROM #{issue_table} WHERE parent_id IS NOT NULL)"
            Rails.logger.info "DEBUG: [sql_for_parent_issue_id_field] Оператор '*', SQL: #{sql}"
            sql
          when "!*"
            # Нет родителя: фильтруем трудозатраты по задачам без parent_id или без задачи вообще
            sql = "(#{time_entry_table}.issue_id IS NULL OR #{time_entry_table}.issue_id IN (SELECT id FROM #{issue_table} WHERE parent_id IS NULL))"
            Rails.logger.info "DEBUG: [sql_for_parent_issue_id_field] Оператор '!*', SQL: #{sql}"
            sql
          else
            "1=1"
